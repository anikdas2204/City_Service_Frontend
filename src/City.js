...
const CityTable = () => {
    ...
    const fetchData = async () => {
        let apiUrl = 'https://temp-app-new-rested-reedbuck-tg.cfapps.sap.hana.ondemand.com/api/v1/cities/list';
 
        if (sortField) {
            apiUrl = `https://temp-app-new-rested-reedbuck-tg.cfapps.sap.hana.ondemand.com/api/v1/cities/listSorted?field=${sortField}&direction=${sortDirection}`;
        }
 
        if (filterName) {
            apiUrl = `https://temp-app-new-rested-reedbuck-tg.cfapps.sap.hana.ondemand.com/api/v1/cities/filterByName?name=${encodeURIComponent(filterName)}`;
        }
